    ...
    public String run() {
        ...
        if (command.equals("")) {
            for (int i = 0; i < gameState.getGameDetails().mapHeight; i++) {
                ...
                if(getEnergy(PlayerType.A) < gameState.getGameDetails().ironCurtainStats.price + getPriceForBuilding(BuildingType.TESLA) || myTeslaOnRow > 0 || enemyTeslaOnRow > 0){
                    if (getEnergy(PlayerType.A) >= gameState.getGameDetails().ironCurtainStats.price && gameState.getPlayers().stream().filter(p -> p.playerType == PlayerType.A).collect(Collectors.toList()).get(0).ironCurtainAvailable && gameState.getPlayers().stream().filter(p -> p.playerType == PlayerType.A).collect(Collectors.toList()).get(0).activeIronCurtainLifetime < 1) {
                        command = "0,0,5";
                        break;
                    }
                }
        ...
        if(gameState.getPlayers().stream().filter(p -> p.playerType == PlayerType.A).collect(Collectors.toList()).get(0).activeIronCurtainLifetime > 3){
            for (int i = 0; i < gameState.getGameDetails().mapWidth/2; i++){
                if(canAffordBuilding(BuildingType.ENERGY)){
                    command = placeBuildingInColumnFromBottom(BuildingType.ENERGY, i);
                    if(!command.equals("")){
                        break;
                    }
                }
        ...
        if (numOfTesla() < 2) {
            for (int i = 0; i < gameState.getGameDetails().mapHeight; i++) {
                ...
                if (getEnergy(PlayerType.A) >= getPriceForBuilding(BuildingType.TESLA) + 0.75*gameState.getGameDetails().ironCurtainStats.price && myAttackOnRow > 0 && myDefenseOnRow > 0){
    ...
    private String placeBuildingRandomlyFromBack(BuildingType buildingType) {
        for (int i = 0; i < gameState.getGameDetails().mapWidth / 2; i++) {
            ...
            if (!listOfFreeCells.isEmpty()) {
                ...
                return buildingType.buildCommand(pickedCell.x, pickedCell.y);
            }
        }
        return "";
    }
